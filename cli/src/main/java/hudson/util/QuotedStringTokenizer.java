...
public class QuotedStringTokenizer extends StringTokenizer {
   ...
   * @param delim            List of delimiter characters as string. Can be null,
   *                         to default to ' \t\n\r'
   * @param returnDelimiters If true, {@link #nextToken()} will include the
   *                         delimiters, not just tokenized tokens.
   * @param returnQuotes     If true, {@link #nextToken()} will include the
   *                         quotation characters when they are present.
   */
  ...
  // FIXME: Bad code this is an extremely verbose way to parse quoted strings
  /* ------------------------------------------------------------ */
  @Override
    ...
    for (int i = 1; i < s.length() - 1; i++) {
      ...
      if(escape) {
        ...
        case 'b':
          ...
          break;
        case 'u':
          b.append((char) ((convertHexDigit((byte) s.charAt(i++)) << 24) + (convertHexDigit((byte) s.charAt(i++)) << 16)
              + (convertHexDigit((byte) s.charAt(i++)) << 8) + (convertHexDigit((byte) s.charAt(i++)))));
          break;
        default:
          b.append(c);
        }
   ...
   *
   * Others, like, say, \W will be left alone instead of becoming just W. This is
   * important to keep Hudson behave on Windows, which uses '\' as the directory
   * separator.
   */

...
public class ApiTokenPropertyTest {
  ...
  public void security49Upgrade() throws Exception {
    ...
    // the replacement for the compromised value must be consistent and cannot be
    // random
  ...
  public void adminsShouldBeUnableToSeeTokensByDefault() throws Exception {
    ...
    HtmlPage  config = wc.goTo(u.getUrl() + "/configure");
    HtmlForm  form   = config.getFormByName("config");
    assertEquals(LocalizedString.ApiTokenProperty_ChangeToken_TokenIsHidden,
        form.getInputByName("_.apiToken").getValueAttribute());
  }
  ...
  public void adminsShouldBeUnableToChangeTokensByDefault() throws Exception {
    ...
    assertEquals("method should not be allowed", HttpURLConnection.HTTP_BAD_METHOD,
        requirePOST.getWebResponse().getStatusCode());

    wc.setThrowExceptionOnFailingStatusCode(true);
    WebRequest request = new WebRequest(
        new URL(j.getURL().toString() + foo.getUrl() + "/" + descriptor.getDescriptorUrl() + "/changeToken"),
        HttpMethod.POST);
    HtmlPage   res     = wc.getPage(request);

    // TODO This nicer alternative requires
    // https://github.com/jenkinsci/jenkins/pull/2268 or similar to work
    // HtmlPage res =
    // requirePOST.getPage().getForms().get(0).getElementsByAttribute("input",
    // "type", "submit").get(0).click();
    assertEquals("Update token response is incorrect", LocalizedString.ApiTokenProperty_ChangeToken_SuccessHidden,
        "<div>" + res.getBody().asText() + "</div>");
  }
  ...
  public void legacyTokenChange() throws Exception {
    ...
    { // but without any legacy token, the direct internal call remains but web UI
      // depends on config
  ...
  private void checkCombinationWithConfigAndMethodForLegacyTokenCreation(ApiTokenPropertyConfiguration config,
      WebClient wc, User user) throws Exception {
    ApiTokenProperty apiTokenProperty = user.getProperty(ApiTokenProperty.class);
    ...
    config.setCreationOfLegacyTokenEnabled(true);
    {
      {// change using web UI
        changeLegacyToken(wc, "user", true);
        String newLegacyToken = apiTokenProperty.getApiToken();
        assertNotEquals(newLegacyToken, LocalizedString.ApiTokenProperty_ChangeToken_CapabilityNotAllowed);
      }
    ...
    config.setCreationOfLegacyTokenEnabled(false);
    {
      {// change not possible using web UI
        ...
        String newLegacyToken = apiTokenProperty.getApiToken();
        assertEquals(newLegacyToken, LocalizedString.ApiTokenProperty_NoLegacyToken);
      }
      revokeLegacyToken(wc, user);
  ...
  private void revokeAllTokenUsingFilter(WebClient wc, User user, Predicate<ApiTokenStore.HashedToken> filter)
      throws Exception {
    ApiTokenProperty apiTokenProperty = user.getProperty(ApiTokenProperty.class);
    List<String>     uuidList         = apiTokenProperty.getTokenStore().getTokenListSortedByName().stream()
        .filter(filter).map(ApiTokenStore.HashedToken::getUuid).collect(Collectors.toList());
  ...
  private void revokeToken(WebClient wc, String login, String tokenUuid) throws Exception {
    WebRequest request = new WebRequest(new URL(j.getURL(),
        "user/" + login + "/descriptorByName/" + ApiTokenProperty.class.getName() + "/revoke/?tokenUuid=" + tokenUuid),
        HttpMethod.POST);
  ...
  private void changeLegacyToken(WebClient wc, String login, boolean success) throws Exception {
    WebRequest request = new WebRequest(
        new URL(j.getURL(),
            "user/" + login + "/descriptorByName/" + ApiTokenProperty.class.getName() + "/changeToken/"),
        HttpMethod.POST);
    Page       p       = wc.getPage(request);
    assertEquals(200, p.getWebResponse().getStatusCode());
    if(success) {
      assertThat(p.getWebResponse().getContentAsString(),
          not(containsString(LocalizedString.ApiTokenProperty_ChangeToken_CapabilityNotAllowed.toString())));
    } else {
      assertThat(p.getWebResponse().getContentAsString(),
          containsString(LocalizedString.ApiTokenProperty_ChangeToken_CapabilityNotAllowed.toString()));
    }
  }
  ...
  private GenerateNewTokenResponse generateNewToken(WebClient wc, String login, String tokenName) throws Exception {
    WebRequest request = new WebRequest(new URL(j.getURL(), "user/" + login + "/descriptorByName/"
        + ApiTokenProperty.class.getName() + "/generateNewToken/?newTokenName=" + tokenName), HttpMethod.POST);
    Page       p       = wc.getPage(request);
    assertEquals(200, p.getWebResponse().getStatusCode());
  ...
  // test no token are generated for new user with the global configuration set to
  // false
}

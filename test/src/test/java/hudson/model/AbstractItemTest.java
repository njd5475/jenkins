...
public class AbstractItemTest {
  ...
  public void checkRenameValidity() throws Exception {
    ...
    assertThat(checkNameAndReturnError(p, ""), equalTo(LocalizedString.Hudson_NoName.toString()));
    assertThat(checkNameAndReturnError(p, ".."), equalTo(LocalizedString.Jenkins_NotAllowedName.toLocale("..")));
    assertThat(checkNameAndReturnError(p, "50%"), equalTo(LocalizedString.Hudson_UnsafeChar.toLocale('%')));
    assertThat(checkNameAndReturnError(p, "foo"), equalTo(LocalizedString.AbstractItem_NewNameUnchanged.toLocale()));
    assertThat(checkNameAndReturnError(p, "foo-exists"),
        equalTo(LocalizedString.AbstractItem_NewNameInUse.toLocale("foo-exists")));

    j.jenkins.setProjectNamingStrategy(new ProjectNamingStrategy.PatternProjectNamingStrategy("bar", "", false));
    assertThat(checkNameAndReturnError(p, "foo1"),
        equalTo(LocalizedString.Hudson_JobNameConventionNotApplyed.toLocale("foo1", "bar")));

    p.scheduleBuild2(0).waitForStart();
    assertThat(checkNameAndReturnError(p, "bar"), equalTo(LocalizedString.Job_NoRenameWhileBuilding.toLocale()));
  }
  ...
  public void checkRenamePermissions() throws Exception {
    ...
    try (ACLContext unused = ACL.as(User.getById("bob", true))) {
      assertThat(checkNameAndReturnError(p, "foo-exists"),
          equalTo(LocalizedString.Jenkins_NotAllowedName.toLocale("foo-exists")));
    }
